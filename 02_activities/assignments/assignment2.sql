...
--- I decided to assign the same number to visits on the same date for each customer
...
-- need to check with LS if this is correct
...
----step 1 & 2 , need to check with LS why mine tomorrow morning why mine isn't working
WITH SalesSummary AS (
    SELECT market_date, SUM(total_sales) AS total_sales
    FROM sales
...
SELECT market_date, total_sales FROM RankedSales WHERE rank_desc = 1
UNION
SELECT market_date, total_sales FROM RankedSales WHERE rank_asc = 1;
...
FROM 
    (SELECT DISTINCT vendor_id, product_id, price FROM vendor_inventory) vi
CROSS JOIN 
